...
class AutoReplyRule(models.Model):
    ...
    TRIGGER_TYPE_CHOICES = [
        ...
        ("dm", "Instagram Direct Message (DM)"),
        ("both", "Both Comments & DMs"),
    ]
    MATCH_TYPE_CHOICES = [
    ...
    trigger_type = models.CharField(
        ...
        default="comment",
        help_text="Where this trigger listens (Comments, DMs, or Both)"
    )
    match_type = models.CharField(
        max_length=20,
        choices=MATCH_TYPE_CHOICES,
        default="contains",
        help_text="Keyword matching logic"
    )
    ...
    is_active = models.BooleanField(default=True, help_text="Enable or pause this automation rule")
    times_triggered = models.IntegerField(default=0, help_text="Number of times this rule has fired")
    last_triggered_at = models.DateTimeField(blank=True, null=True, help_text="Timestamp of last execution")
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.user.username}: {self.trigger_keyword} -> {self.reply_text[:30]} ({self.trigger_type} / {self.reply_type})"


class InstagramConversation(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='ig_conversations')
    account = models.ForeignKey(InstagramAccount, on_delete=models.CASCADE, related_name='conversations', null=True, blank=True)
    conversation_id = models.CharField(max_length=100, unique=True, help_text="Meta IG Conversation ID or Thread ID")
    participant_id = models.CharField(max_length=100, help_text="Instagram User Scoped ID (IGSID)")
    participant_username = models.CharField(max_length=100, blank=True, null=True)
    participant_name = models.CharField(max_length=150, blank=True, null=True)
    participant_profile_pic = models.URLField(max_length=1000, blank=True, null=True)
    ...
    def __str__(self):
        name = self.participant_username or self.participant_name or self.participant_id
        return f"Conversation with {name} ({self.user.username})"


class InstagramDirectMessage(models.Model):
    conversation = models.ForeignKey(InstagramConversation, on_delete=models.CASCADE, related_name='messages')
    message_id = models.CharField(max_length=150, unique=True, help_text="Meta IG Message ID (mid.xxx)")
    sender_id = models.CharField(max_length=100, help_text="IGSID of message sender")
    ...
    def __str__(self):
        direction = "Outgoing" if self.is_from_business else "Incoming"
        return f"[{direction}] {self.conversation.participant_username or self.conversation.participant_id}: {(self.text or '')[:30]}"
